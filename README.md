...
#### iPhone / iPad（Safari）
> ⚠️ iOS 上**只有 Safari** 才能把网页添加到主屏幕，微信 / Chrome 内打开时请先点「在 Safari 中打开」。

1. 打开 **Safari**，访问 [https://pink-workbench.pages.dev](https://pink-workbench.pages.dev)。
...
#### Android（Chrome / Edge）
1. 用 Chrome 或 Edge 打开 [https://pink-workbench.pages.dev](https://pink-workbench.pages.dev)。
...
入口：**治愈花园 → 我的宠物 → 出去玩**。花园（15 分钟 / 10 爱心）、咖啡馆（30 分钟 / 18 爱心）、森林和健身房（1 小时 / 30 爱心）、商场（2 小时 / 50 爱心）、海边（3 小时 / 70 爱心）全部免费开放。
...
- `pet-outings-core.js`：场景、时长、奖励与行程规则。
- `pet-outings.js` / `pet-outings.css`：场景选择、倒计时、查看与领奖界面。
- `generated-images/outing-*-v1.png`：六张内置 image_gen 生成的背景，已纳入版本管理范围。
- `tools/outing-image-prompts.json`：完整生图提示词与输出路径。
